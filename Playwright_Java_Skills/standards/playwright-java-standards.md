...
```text
public void clickLogin() { page.locator(LOGIN_BUTTON).click(); }
```

Example assertion:

```text
assertThat(page.locator(DASHBOARD_HEADING)).isVisible();
```
...
- Every automated case must include a TC-ID and suite marker/tag: `@Tag("smoke") or TestNG groups`.
...
@Test
@Tag("smoke")
void TC_LGN_001_loginWithValidCredentials() { loginPage.loginAs("Admin"); }
```
