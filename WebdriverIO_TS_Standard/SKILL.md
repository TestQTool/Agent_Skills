---
name: WebdriverIO_TS_Standard
description: Generate WebdriverIO TypeScript Standard automation from explicitly selected, approved test cases. Use for Qentrix automation-script generation against the matching static framework; never use this skill to create test cases or invent business scenarios.
---

# WebdriverIO TypeScript Standard Script Generation

Generate runnable, framework-compatible automation for the existing `WebdriverIO_TS_Standard` stack. Treat selected approved test cases as the only behavioral source of truth.
...
This skill folder is WebdriverIO_TS_Standard for WebdriverIO / TypeScript / Standard. Use updateagentskill/playwright/javascript/hybrid as a quality reference only; preserve this skill's own tool, language, framework type, folder name, and output conventions.
...
Mapped updated static framework: D:\frameworks\StaticFrameworks\Web Automation\webdriverio\typescript\standard

- Run command: npx wdio run wdio.conf.ts
- Test pattern: test/specs/**/*.ts
...
From standards\webdriverio-typescript-standards.md:
- # WebdriverIO TypeScript Framework Coding Standards
...
From CLAUDE.md:
- # QA Automation - WebdriverIO TypeScript Framework Project Memory
- ## Goal
- Generate a self-contained `webdriverio-typescript` web automation framework that a user can clone, install, configure, and run locally or in CI.
- ## Runtime Layout
...
- - `test/specs/<feature>.spec.ts`: test orchestration only.
