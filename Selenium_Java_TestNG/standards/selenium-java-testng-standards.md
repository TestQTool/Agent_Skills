...
```text
Assert.assertEquals(getText(SUCCESS_MESSAGE), expectedMessage, "Success message mismatch");
```
...
- Every automated case must include a TC-ID and suite marker/tag: `@Test(groups={"smoke"})`.
