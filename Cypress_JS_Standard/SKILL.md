...
Generate runnable, framework-compatible automation for the existing `Cypress_JS_Standard` stack. Treat selected approved test cases as the only behavioral source of truth.
...
This skill folder is Cypress_JS_Standard for Cypress / JavaScript / Standard. Use updateagentskill/playwright/javascript/hybrid as a quality reference only; preserve this skill's own tool, language, framework type, folder name, and output conventions.
...
Mapped updated static framework: D:\frameworks\StaticFrameworks\Web Automation\cypress\javascript\standard
...
From standards\cypress-js-standards.md:
...
From CLAUDE.md:
- # QA Automation - Cypress JavaScript Framework Project Memory
- ## Goal
- Generate a self-contained `cypress-js` web automation framework that a user can clone, install, configure, and run locally or in CI.
- ## Runtime Layout
- ## Framework Architecture
- ## Generated Files Per Feature
- - `cypress/pageObjects/<feature>PageObjects.js`: locators/selectors only.
- - `cypress/pages/<feature>Page.js`: page actions, waits, data helpers, and assertions.
- - `cypress/e2e/<feature>.cy.js`: test orchestration only.
