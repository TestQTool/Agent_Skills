---
name: Appium_TS_WebdriverIO
description: Generate Appium TypeScript WebdriverIO automation from explicitly selected, approved test cases. Use for Qentrix automation-script generation against the matching static framework; never use this skill to create test cases or invent business scenarios.
---

# Appium TypeScript WebdriverIO Script Generation

Generate runnable, framework-compatible automation for the existing `Appium_TS_WebdriverIO` stack. Treat selected approved test cases as the only behavioral source of truth.
...
This skill folder is Appium_TS_WebdriverIO for Appium / TypeScript / Standard. Use updateagentskill/playwright/javascript/hybrid as a quality reference only; preserve this skill's own tool, language, framework type, folder name, and output conventions.
...
From standards\appium-webdriverio-typescript-standards.md:
- # Appium TypeScript WebdriverIO Framework Mobile Standards
...
From CLAUDE.md:
- # Mobile Automation - Appium TypeScript WebdriverIO Framework Project Memory
- ## Goal
- Generate a self-contained `appium-webdriverio-typescript` mobile automation framework that supports both Android and iOS.
- ## Runtime Layout
...
- - `test/screenobjects/<feature>.selectors.ts`: Android and iOS locators/selectors only.
- - `test/screenobjects/<feature>.screen.ts`: screen actions, waits, gestures, and assertions.
