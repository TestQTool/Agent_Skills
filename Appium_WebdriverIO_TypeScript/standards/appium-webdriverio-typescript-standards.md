...
```text
export const loginButton: PlatformSelector = { android: '~login', ios: '-ios predicate string:name == "login"' };
```
...
```text
async tapLogin(): Promise<void> { await $(platformSelector(loginButton)).click(); }
```
