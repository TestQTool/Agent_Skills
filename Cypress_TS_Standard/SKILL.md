---
name: Cypress_TS_Standard
description: Generate Cypress TypeScript Standard automation from explicitly selected, approved test cases. Use for Qentrix automation-script generation against the matching static framework; never use this skill to create test cases or invent business scenarios.
---

# Cypress TypeScript Standard Script Generation

Generate runnable, framework-compatible automation for the existing `Cypress_TS_Standard` stack. Treat selected approved test cases as the only behavioral source of truth.
...
This skill folder is Cypress_TS_Standard for Cypress / TypeScript / Standard. Use updateagentskill/playwright/javascript/hybrid as a quality reference only; preserve this skill's own tool, language, framework type, folder name, and output conventions.
...
Mapped updated static framework: D:\frameworks\StaticFrameworks\Web Automation\cypress\typescript\standard

- Run command: npx cypress run
- Test pattern: cypress/e2e/**/*.cy.ts
...
- # Cypress TypeScript Build Scripts Skill
- # Purpose: Convert approved test cases into runnable Cypress TypeScript Framework files.
- ## Role
- Generate production-ready automation for a user-owned repository. The output must plug into the static `cypress-typescript` framework and run on the user's machine.
- ## Inputs You May Receive
- - Selected feature or requirement name
- - Approved manual test cases and steps
- - Application context
- - Framework memory from `CLAUDE.md`
- - Coding standards from `standards/cypress-typescript-standards.md`
- - Static framework context from `StaticFrameworks/cypress-typescript`
...
- # Cypress TypeScript Explore Skill
- ## Role
- ## Modes
- ### Module Discovery
...
From standards\cypress-typescript-standards.md:
- # Cypress TypeScript Framework Coding Standards
...
From CLAUDE.md:
- # QA Automation - Cypress TypeScript Framework Project Memory
- ## Goal
- Generate a self-contained `cypress-typescript` web automation framework that a user can clone, install, configure, and run locally or in CI.
- ## Runtime Layout
...
- - `cypress/e2e/<feature>.cy.ts`: test orchestration only.
