---
name: RobotFramework_Python_SeleniumLibrary
description: Generate RobotFramework Python Seleniumlibrary automation from explicitly selected, approved test cases. Use for Qentrix automation-script generation against the matching static framework; never use this skill to create test cases or invent business scenarios.
---

# RobotFramework Python Seleniumlibrary Script Generation

Generate runnable, framework-compatible automation for the existing `RobotFramework_Python_SeleniumLibrary` stack. Treat selected approved test cases as the only behavioral source of truth.
...
7. Map every approved expected result to a retrying assertion/check supported by `RobotFramework`.
...
This skill folder is RobotFramework_Python_SeleniumLibrary for Robot Framework / Unspecified / SeleniumLibrary. Use updateagentskill/playwright/javascript/hybrid as a quality reference only; preserve this skill's own tool, language, framework type, folder name, and output conventions.
...
Mapped updated static framework: D:\frameworks\StaticFrameworks\Web Automation\robotframework\python\selenium-library
...
From standards\robotframework-seleniumlibrary-standards.md:
...
From CLAUDE.md:
- # QA Automation - Robot Framework Robot Selenium Library Project Memory
- ## Goal
- Generate a self-contained `robotframework-seleniumlibrary` web automation framework that a user can clone, install, configure, and run locally or in CI.
- ## Runtime Layout
- ## Framework Architecture
- ## Generated Files Per Feature
- - `resources/locators/<feature>_locators.resource`: locators/selectors only.
- - `resources/pages/<feature>_keywords.resource`: page actions, waits, data helpers, and assertions.
