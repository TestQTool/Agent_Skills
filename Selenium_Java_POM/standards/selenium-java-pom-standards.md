...
```text
public void loginAs(String role) { LoginData data = getLoginDataByRole(role); type(USERNAME_INPUT, data.username()); type(PASSWORD_INPUT, data.password()); click(LOGIN_BUTTON); }
```

Example assertion:

```text
assertThat(getText(DASHBOARD_HEADING)).contains("Dashboard");
```
...
- Every automated case must include a TC-ID and suite marker/tag: `smoke/regression test groups or tags`.
