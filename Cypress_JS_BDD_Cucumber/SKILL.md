---
name: Cypress_JS_BDD_Cucumber
description: Generate Cypress JavaScript BDD Cucumber automation from explicitly selected, approved test cases. Use for Qentrix automation-script generation against the matching static framework; never use this skill to create test cases or invent business scenarios.
---

# Cypress JavaScript BDD Cucumber Script Generation

Generate runnable, framework-compatible automation for the existing `Cypress_JS_BDD_Cucumber` stack. Treat selected approved test cases as the only behavioral source of truth.
...
This skill folder is Cypress_JS_BDD_Cucumber for Cypress / Unspecified / BDD Cucumber. Use updateagentskill/playwright/javascript/hybrid as a quality reference only; preserve this skill's own tool, language, framework type, folder name, and output conventions.
...
Mapped updated static framework: D:\frameworks\StaticFrameworks\Web Automation\cypress\javascript\bdd

- Run command: npx cypress run
- Test pattern: cypress/e2e/**/*.feature
...
- # Cypress JavaScript/TypeScript Build Scripts Skill
- # Purpose: Convert approved test cases into runnable Cypress JavaScript/TypeScript BDD Cucumber Framework files.
- ## Role
- Generate production-ready automation for a user-owned repository. The output must plug into the static `cypress-bdd-cucumber` framework and run on the user's machine.
- ## Inputs You May Receive
- - Selected feature or requirement name
- - Approved manual test cases and steps
- - Application context
- - Framework memory from `CLAUDE.md`
- - Coding standards from `standards/cypress-bdd-cucumber-standards.md`
- - Static framework context from `StaticFrameworks/cypress-bdd-cucumber`
...
- # Purpose: Ensure generated automation output is a complete cypress-bdd-cucumber framework that users can clone and run locally.
- ## Role
- ## Required Runtime Files
- ## Merge Rules
- 1. Copy missing static framework files from `StaticFrameworks/cypress-bdd-cucumber`.
...
From standards\cypress-bdd-cucumber-standards.md:
- # Cypress JavaScript/TypeScript BDD Cucumber Framework Coding Standards
...
From CLAUDE.md:
- # QA Automation - Cypress JavaScript/TypeScript BDD Cucumber Framework Project Memory
- ## Goal
- Generate a self-contained `cypress-bdd-cucumber` web automation framework that a user can clone, install, configure, and run locally or in CI.
- ## Runtime Layout
- ## Framework Architecture
- ## Generated Files Per Feature
- - `cypress/pageObjects/<feature>PageObjects.ts`: locators/selectors only.
- - `cypress/pages/<feature>Page.ts`: page actions, waits, data helpers, and assertions.
- - `cypress/e2e/step_definitions/<feature>Steps.ts`: test orchestration only.
