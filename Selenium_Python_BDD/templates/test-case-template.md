...
| Module / Feature Area | Feature name and feature tag |
| Priority P1 | `@smoke` |
| Priority P2+ | `@regression` |
| Preconditions | Background or Given steps |
| Test Steps | Given / When steps |
| Expected Results | Then steps |
| Test Data | Examples table for Scenario Outline |
...
Scenario: Admin can apply for annual leave successfully
  ...
  And the admin enters leave dates from "2025-08-01" to "2025-08-05"
...
| When | `the {role} enters {field} as "{value}"` | `the admin enters employee name as "Asha Rao"` |
