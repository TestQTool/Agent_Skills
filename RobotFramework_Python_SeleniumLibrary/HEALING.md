# RobotFramework Python Seleniumlibrary Run Healing

Use this file when generated `RobotFramework_Python_SeleniumLibrary` automation fails during Run Automation. These rules repair generated client automation only. They do not create new business test cases and they do not modify reference framework files.

## Scope

Apply these healing rules only when all are true:

- Tool/framework is `RobotFramework`.
- Language is `Python`.
- Framework type is `Seleniumlibrary`.
...
This healing file is for RobotFramework_Python_SeleniumLibrary using Robot Framework / Unspecified / SeleniumLibrary. Use updateagentskill/playwright/javascript/hybrid as a quality reference only; preserve this tool's own repair patterns and generated file zones.
